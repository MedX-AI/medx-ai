...
// Custom hook for letter jumbling effect
const useJumbleText = (text) => {
  const [jumbledText, setJumbledText] = useState(text);
  const [isJumbling, setIsJumbling] = useState(false);

  const jumbleLetters = useCallback(() => {
  ...
  useEffect(() => {
    let intervalId;
    if (isJumbling) {
      intervalId = setInterval(jumbleLetters, 50); // Adjust interval as needed
    } else {
      setJumbledText(text);
    }
    return () => {
  ...
  }, [isJumbling, jumbleLetters, text]);

  return [jumbledText, setIsJumbling];
};

const NavItem = ({ href, text }) => {
  const [jumbledText, setIsJumbling] = useJumbleText(text);
  return (
    <a
      href={href}
      className="text-sm hover:text-gray-300 transition-colors duration-200"
      onMouseEnter={() => setIsJumbling(true)}
      onMouseLeave={() => setIsJumbling(false)}
    >
      {jumbledText}
    </a>
  );
